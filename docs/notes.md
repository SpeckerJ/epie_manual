...
The final sections repeat the ePiE calculations under three long-term hydrological conditions:

    average flow;
    maximum flow;
    minimum flow.

For each flow scenario, the model is run for every API. The resulting node-level predictions are combined into one dataset, with an additional scenario column identifying the hydrological condition. Concentration statistics can then be compared among flow scenarios to assess how river flow affects predicted environmental concentrations.

The final example extends this approach to multiple river basins. ePiE is run for every combination of selected basin, API, and flow scenario, after which mean and median predicted concentrations are summarised by API, basin, and scenario.
